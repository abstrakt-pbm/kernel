...
void LocalAPIC::setInitialTimerCount(uint32_t timerCount) {
  ...
  uint32_t ind =
      static_cast<uint32_t>(LAPIC_OFFSETS::TIMER_INITIAL_COUNT_REGISTER) / 4;
...
void LocalAPIC::setTimerDivideConfiguration(uint32_t timerDivide) {
  ...
  uint32_t ind = static_cast<uint32_t>(
                     LAPIC_OFFSETS::TIMER_DIVIDE_CONFIGURATION_REGISTER) /
                 4;
  lapic_base_[ind] = timerDivide;
}

void LocalAPIC::setupTimerLVT(uint8_t idt_vector, uint8_t delivery_mode,
                              uint8_t mask, LAPIC_TIMER_MODE timer_mode) {
  uint32_t lvt_timer_value = idt_vector | (delivery_mode << 8) | (mask << 16) |
