...
void *KernelBufferAllocator::allocate(uint64_t length) {
  ...
  if (ptr) {
    BufferInfo *new_buffer_info = new BufferInfo(
        ptr, allocation_length, BufferSource::KOA, nullptr, buffer_info_head_);
...
void KernelBufferAllocator::free(void *ptr, size_t obj_size) {
  ...
  while (current_buffer_info) {
    if (current_buffer_info->source_ == BufferSource::KOA &&
        current_buffer_info->ptr_ == ptr) {
      koa.free(ptr, current_buffer_info->length_);
