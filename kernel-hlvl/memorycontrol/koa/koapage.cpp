...
KOAPage::KOAPage(KOAPage *next_koa_page, uint64_t object_size)
    : next_koa_page_(next_koa_page), object_size_(object_size), length_(0) {

  Address free_object_area_start =
      reinterpret_cast<Address>(this) + sizeof(KOAPage);
  ...
  *reinterpret_cast<size_p *>(free_object_area_start +
                              (capacity_)*object_size_) = 0;
}
