...
VFS::VFS(FS::Dirrectory *root_dir)
: root_dir_(root_dir){
}
		...
		for (const auto& fsnode : *current_parentnode) {
			if (fsnode->name() == path_tokens[i]) {
				if (i < path_tokens.size() -1) {
					current_parentnode = observer_ptr<Dirrectory>(
						static_cast<Dirrectory*>(
							fsnode.get()));
					continue;
				} else {
...
void VFS::mkfile(Path path, unique_ptr<FSNode> node) {
	...
	observer_ptr<FSNode> parent_dir = resolve_path(
...
String Path::filename() const {
	return path_tokens_[path_tokens_.size() - 1];
}

bool Path::had_parent() const {
	return true;
}
